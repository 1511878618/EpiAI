...
class XGBSingleForecaster:
    ...
    def fit(self, x, y, val_x=None, val_y=None, verbose=False):
        ...
        if val_x is not None and val_y is not None:
            X_val = self._flatten_x(val_x)
            y_val = self._prepare_y(val_y).reshape(val_y.shape[0], -1)

            self.model.fit(
                X,
                y_flat,
                eval_set=[(X, y_flat), (X_val, y_val)],
                verbose=verbose,
            )
        else:
